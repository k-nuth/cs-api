...
namespace Knuth.Tests {
    ...
    public class ConfigTest : IClassFixture<NodeFixture> {
        ...
        public async Task TestDefaultSettings() {
            ...
            // Assert.Equal(config.Chain.ByteFeeSatoshis).toBeCloseTo(0.1);
            ...
            // Assert.Equal(config.Chain.Checkpoints.Count).toBeGreaterThanOrEqual(64);
            Assert.Equal(config.Chain.Checkpoints[0].Height, 0UL);
            // Assert.Equal(enc.Hash.bytesToStr(config.Chain.Checkpoints[0].hash), 
